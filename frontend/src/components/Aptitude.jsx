...
function Aptitude() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await axios.get(`http://127.0.0.1:8000/v1/aptitude/user-report/${userId}`);
      setReport(res.data);
    } catch (err) {
    ...
    <div className="h-screen flex flex-col overflow-hidden gap-[1.5rem]">
      ...
      <div className="absolute right-9 top-26 h-[600px] w-[22%] bg-white border border-gray-200 rounded-2xl shadow-lg p-6 flex flex-col gap-6">
        ...
        <div className="mt-auto flex flex-col gap-3">
          <button onClick={handleStartSession} className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition cursor-pointer">
            Retake Test
          </button>
      ...
      <div className="flex flex-col gap-8 w-[70%] flex-1 overflow-auto">
        ...
        <div className=" relative top-20   flex gap-3 items-center ">
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "all" ? "bg-gray-300" : "bg-white"
          ...
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "science" ? "bg-gray-300" : "bg-white"
              } border-[#0005] p-1 px-4 rounded-md`}
          >
            Science
          </button>
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "commerce" ? "bg-gray-300" : "bg-white"
              } border-[#0005] p-1 px-4 rounded-md`}
          >
            Commerce
          </button>
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "arts" ? "bg-gray-300" : "bg-white"
              } border-[#0005] p-1 px-4 rounded-md`}
          >
            Arts
          </button>
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "medical" ? "bg-gray-300" : "bg-white"
              } border-[#0005] p-1 px-4 rounded-md`}
          >
            Medical
          </button>
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "architecture" ? "bg-gray-300" : "bg-white"
              } border-[#0005] p-1 px-4 rounded-md`}
          >
            Architecture
          </button>
          <button
            ...
            className={`border-1 cursor-pointer  ${category == "nursing" ? "bg-gray-300" : "bg-white"
              } border-[#0005] p-1 px-4 rounded-md`}
          >
            Nursing
          </button>
        ...
        <div className="relative top-20  pb-7">
          <CareerQuestions onselect={category} currentUser={currentUser} onTestComplete={fetchReport} />
        </div>
      </div>
              ...
              <div className="mt-10 flex flex-col gap-5 items-start w-full ">
                ...
                <div className="space-y-6 flex flex-col gap-4 w-full">
                  {results.results.map((r, i) => (
                    <div
                      key={r.question_id}
                      className={`p-5 rounded-xl flex flex-col gap-2 shadow-md border-l-4 transition-all ${r.is_correct ? "bg-green-50 border-green-500" : "bg-red-50 border-red-500"
                        }`}
                    >
                      ...
                      <p className="text-gray-700">
                        ...
                        <span
                          className={`${r.is_correct ? "text-green-700 font-semibold" : "text-red-700 font-semibold"
                            }`}
                        >
                          {r.selected_option}
                        </span>
                  ...
                  >
                    ...
                    <div className="space-y-3 flex flex-col gap-2">
                      {q.options.map((opt, idx) => (
                        <label
                          key={idx}
                          className={`flex items-center  p-3 border rounded-lg cursor-pointer transition-all ${answers[q.id] === opt
